...
// Global scaling factors applied to MoveGroup trajectory generation.
...
{
    // Default to MoveGroup plan execution unless a Cartesian trajectory is computed.
    is_trajectory_ = false;
    move_group_ = std::make_shared<moveit::planning_interface::MoveGroupInterface>(node_, group_name);

    // Log useful runtime context for debugging robot model / group configuration.
    ...
    // Apply default speed and acceleration scaling to all generated plans.
...
{   
    ...
    // Compute a straight-line Cartesian path through waypoints.
    double fraction = move_group_->computeCartesianPath(pose_target_vector, eef_step, jump_threshold, trajectory_);
    bool success = true;

    // Require near-complete path generation to avoid partial, unsafe execution.
    if(fraction < 0.9) {
        RCLCPP_ERROR(node_->get_logger(), "planCartesianPath: plan failed, fraction=%lf", fraction);
        return false;
    }

    // Mark that execution should use `trajectory_` instead of `xarm_plan_`.
    is_trajectory_ = true;
...
bool XArmPlanner::executePath(bool wait)
{
    ...
    // Execute synchronously or asynchronously based on caller preference,
    // and select the correct plan container based on the last planning mode.
    if (wait)
